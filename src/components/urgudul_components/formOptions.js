...
function FormOptions({ label, options, values, value, name, index, setter, classAppend, classLabel, invalid }) {
    ...
    return (
        <div className={`tw-relative tw-px-3 tw-pt-8 tw-pb-3 tw-flex tw-items-center ${classAppend}`}>
            <label className={`tw-absolute tw-px-1 tw-rounded-full tw-whitespace-nowrap ${invalid && 'tw-text-red-500'} tw-transition-colors tw-duration-300 ${classLabel} tw-text-sm tw-top-4 tw-left-3`}>
                {label}
            </label>

            <div className="tw-flex tw-flex-col tw-items-start tw-pt-2 tw-pl-3">
                {options?.map((item, i) =>
