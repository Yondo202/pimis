...
function FormInline(props) {
    ...
    return (
        <div className={`tw-relative tw-px-3 tw-pt-8 tw-pb-3.5 tw-flex tw-items-center ${props.classAppend}`}>
            <label className={`tw-absolute tw-px-1 ${props.classLabel ? props.classLabel : 'tw-bg-white'} tw-rounded-full tw-whitespace-nowrap tw-font-light ${focused ? 'tw-text-sm tw-top-2 tw-left-8' : 'tw-text-xs tw-top-6 tw-left-12'} tw-transition-all tw-duration-300`}>
                {props.label}
            </label>

            <PenSVG className={`tw-w-5 tw-h-5 tw-flex-shrink-0 ${props.invalid ? 'tw-text-red-500' : 'tw-text-gray-500'} tw-transition-colors`} />

            {props.type === 'numberFormat' ?
                <NumberFormat className={`tw-h-8.5 tw-ml-2 tw-text-sm tw-bg-transparent tw-border ${props.invalid ? 'tw-border-red-500' : 'tw-border-gray-400'} tw-rounded-sm tw-pt-2 tw-pb-1 tw-px-2 tw-outline-none tw-transition-colors tw-placeholder-gray-400 ${props.classInput}`} {...props.formats} value={props.value} onValueChange={(values) => props.onChange(values, props.name, props.id)} placeholder={props.placeholder} onFocus={() => setFocused(true)} onBlur={() => setFocused(false)} />
                :
