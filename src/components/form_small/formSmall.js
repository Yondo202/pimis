            ...
            <div className="tw-py-3 tw-px-4 tw-flex-grow sm:tw-w-7/12">
                {
                    props.type === "numberFormat" ?
                        <NumberFormat className="tw-w-24 tw-bg-transparent tw-text-sm tw-text-gray-700 tw-outline-none tw-border tw-border-gray-300 tw-rounded-md tw-py-2 tw-px-3 focus:tw-border-blue-600 tw-transition tw-duration-300 tw-placeholder-gray-700 tw-placeholder-opacity-50 focus:tw-placeholder-opacity-70" thousandSeparator={true} value={props.value} name={props.name} id={props.id} onChange={props.onChange} placeholder={props.placeholder} onFocus={() => setFocused(true)} onBlur={() => setFocused(false)} />
                        :
