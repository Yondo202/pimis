...
export default function ModalWindow({ modalOpen, setModalOpen, children }) {
         ...
         {item => item && (anims =>
            <animated.div className="tw-fixed tw-top-0 tw-left-0 tw-w-screen tw-h-screen tw-bg-gray-700 tw-bg-opacity-80 tw-z-10 tw-flex tw-justify-center tw-items-center" style={anims}>
               <Transition
                  ...
                  {item1 => item1 && (anims1 =>
                     <animated.div style={anims1} className="tw-bg-white tw-relative tw-rounded tw-shadow-md tw-p-2 tw-m-2" ref={modalRef}>
                        {children}
                     </animated.div>
                  )}
               </Transition>
