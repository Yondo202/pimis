...
export default function RegistrationsModal(props) {
    const showModal = props.registrationsModalOpen
    const setModal = props.setRegistrationsModalOpen
                        ...
                        {item1 => item1 && (anims1 =>
                            <div style={anims1} className="tw-bg-white tw-relative tw-rounded-md tw-shadow-lg tw-p-4 tw-m-8 tw-w-full tw-max-w-3xl tw-flex tw-flex-col tw-items-center" ref={modalRef}>
                                ...
                                <div className="tw-flex tw-flex-wrap tw-justify-start">
                                    {props.registrations.map(item =>
                                        <button className="tw-w-32 tw-h-40 tw-rounded-md tw-shadow-md tw-border tw-m-3 tw-transform-gpu hover:tw-scale-105 tw-transition-all tw-duration-300 focus:tw-outline-none tw-inline-flex tw-flex-col" key={item.id} onClick={() => props.handleSelectRegistration(item.id)}>
