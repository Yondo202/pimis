...
export default function TrainingNavigatorAdmin() {
   ...
   const transitionsPages = useTransition(location, location => location.pathname, {
      from: { opacity: 0, transform: location.pathname === '/training' ? 'translateX(-200px)' : 'translateX(200px)' },
      enter: { opacity: 1, transform: 'translateX(0)' },
      leave: { opacity: 0, transform: location.pathname === '/training' ? 'translateX(200px)' : 'translateX(-200px)' },
      initial: { opacity: 1 },
      config: config.stiff,
   })
