                    ...
                    <li className="tw-p-2 tw-pl-4 tw-font-medium odd:tw-bg-gray-50" key={item.code}>
                        ...
                        <div className="tw-flex tw-flex-wrap tw-mt-1">
                            {item.files?.map((file, j) =>
                                <FileCard name={file.name} type={file.mimetype} size={file.size} removeFile={() => handleRemoveFile(item.code, j)} classAppend="tw-my-1 tw-mx-1.5" uploading={file === 'loading' && true} downloadFile={() => handleDownloadFile(item.code, j)} />
                            )}

                            <button className="tw-inline-flex tw-items-center tw-rounded-lg tw-my-1 tw-mx-1.5 focus:tw-outline-none" onClick={() => handleButtonClick(item.code)}>
