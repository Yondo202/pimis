                ...
                <thead className="">
                    <tr className="tw-h-16">
                        ...
                        <th className="tw-px-4 tw-sticky tw-top-0 tw-bg-gray-100">
                            <button className="tw-flex tw-items-center tw-text-sm tw-font-bold tw-uppercase tw-text-gray-500 tw-rounded-md focus:tw-outline-none active:tw-bg-gray-200 tw-py-2 tw-px-2" onClick={() => sortBy('description_mon')}>
                                <span className={`${sort.by === 'description_mon' && 'tw-text-blue-500'} tw-transition tw-duration-300`}>Бүтээгдэхүүн</span>
                                <ArrowSVG className={`tw-w-4 tw-h-4 tw-transform ${sort.by === 'description_mon' ? (sort.asc ? 'tw-rotate-90 tw-text-blue-600' : 'tw--rotate-90 tw-text-blue-600') : 'tw-rotate-90 tw-animate-pulse'}`} />
                            </button>
                        </th>
                        <th className="tw-px-4 tw-sticky tw-top-0 tw-bg-gray-100">
                            <button className="tw-flex tw-items-center tw-text-sm tw-font-bold tw-uppercase tw-text-gray-500 tw-rounded-md focus:tw-outline-none active:tw-bg-gray-200 tw-py-2 tw-px-2" onClick={() => sortBy('pcode')}>
                                <span className={`${sort.by === 'pcode' && 'tw-text-blue-500'} tw-transition tw-duration-300`}>Код</span>
                                <ArrowSVG className={`tw-w-4 tw-h-4 tw-transform ${sort.by === 'pcode' ? (sort.asc ? 'tw-rotate-90 tw-text-blue-600' : 'tw--rotate-90 tw-text-blue-600') : 'tw-rotate-90 tw-animate-pulse'}`} />
                            </button>
                        </th>
                        <th className="tw-px-4 tw-sticky tw-top-0 tw-bg-gray-100">
                            <button className="tw-flex tw-items-center tw-text-sm tw-font-bold tw-uppercase tw-text-gray-500 tw-rounded-md focus:tw-outline-none active:tw-bg-gray-200 tw-py-2 tw-px-2" onClick={() => sortBy('description')}>
                                <span className={`${sort.by === 'description' && 'tw-text-blue-500'} tw-transition tw-duration-300`}>Product</span>
                                <ArrowSVG className={`tw-w-4 tw-h-4 tw-transform ${sort.by === 'description' ? (sort.asc ? 'tw-rotate-90 tw-text-blue-600' : 'tw--rotate-90 tw-text-blue-600') : 'tw-rotate-90 tw-animate-pulse'}`} />
                            </button>
                        </th>
                    </tr>
                </thead>
                <tbody className="tw-divide-y tw-divide-dashed">
                    {
                        products.sort(compare).filter(filter).map(item => (
                            <tr className="tw-bg-white tw-text-sm tw-h-20" key={item.id}>
                                ...
                                <td className="tw-text-center">{item.pcode}</td>
            ...
            <div className={`tw-fixed tw-top-0 tw-left-0 tw-w-screen tw-h-screen tw-flex tw-justify-center tw-items-center tw-bg-gray-700 tw-bg-opacity-70 ${modal.open ? 'tw-visible tw-opacity-100' : 'tw-invisible tw-opacity-0'} tw-transition-all tw-duration-300`}>
                <div className="tw-bg-white tw-relative tw-rounded-md tw-shadow-lg tw-p-4 tw-m-2 tw-w-full tw-max-w-3xl">
                    ...
                    <div className="tw-mt-8 tw-mb-2 tw-flex tw-flex-col">
                        <h3 className="tw-text-xl tw-font-bold tw-text-center tw-mb-4">{modal.id ? 'Засварлах' : 'Шинээр нэмэх'}</h3>
                    ...
                    <div className="tw-flex tw-flex-col sm:tw-flex-row tw-items-center tw-mb-4">
                        <label className="tw-w-full sm:tw-w-40 tw-min-w-min tw-pl-2 sm:tw-text-center tw-font-semibold">Бүтээгдэхүүн:</label>
                        <input className="tw-w-full sm:tw-flex-grow tw-min-w-min tw-max-w-lg tw-text-sm tw-bg-transparent tw-rounded-md tw-py-2 tw-px-2 tw-border tw-border-gray-300 tw-outline-none focus:tw-border-blue-600 tw-transition tw-duration-300" type="text" value={temp.description_mon} name="description_mon" onChange={handleInput} />
                    </div>
                    <div className="tw-flex tw-flex-col sm:tw-flex-row tw-items-center tw-mb-4">
                        <label className="tw-w-full sm:tw-w-40 tw-min-w-min tw-pl-2 sm:tw-text-center tw-font-semibold">Код:</label>
                        <input className="tw-w-full sm:tw-flex-grow tw-min-w-min tw-max-w-lg tw-text-sm tw-bg-transparent tw-rounded-md tw-py-2 tw-px-2 tw-border tw-border-gray-300 tw-outline-none focus:tw-border-blue-600 tw-transition tw-duration-300" type="text" value={temp.pcode} name="description_mon" onChange={handleInput} />
                    </div>
                    <div className="tw-flex tw-flex-col sm:tw-flex-row tw-items-center tw-mb-4">
                        <label className="tw-w-full sm:tw-w-40 tw-min-w-min tw-pl-2 sm:tw-text-center tw-font-semibold">Бүтээгдэхүүн англиар:</label>
                        <input className="tw-w-full sm:tw-flex-grow tw-min-w-min tw-max-w-lg tw-text-sm tw-bg-transparent tw-rounded-md tw-py-2 tw-px-2 tw-border tw-border-gray-300 tw-outline-none focus:tw-border-blue-600 tw-transition tw-duration-300" type="text" value={temp.description} name="description" onChange={handleInput} />
                    </div>
            ...
            <div className={`tw-fixed tw-top-0 tw-left-0 tw-w-screen tw-h-screen tw-flex tw-justify-center tw-items-center tw-bg-gray-700 tw-bg-opacity-70 ${confirm.open ? 'tw-visible tw-opacity-100' : 'tw-invisible tw-opacity-0'} tw-transition-all tw-duration-300`}>
                <div className="tw-bg-white tw-relative tw-rounded-md tw-shadow-lg tw-p-4 tw-m-2 tw-w-full tw-max-w-xl tw-flex tw-flex-col tw-border-2 tw-border-red-600 tw-border-opacity-80">
                    ...
                    <p className="tw-mb-6 tw-font-semibold">
                        <span className="tw-mr-1 tw-text-red-500">"{products.filter(obj => obj.id === confirm.id)[0] && products.filter(obj => obj.id === confirm.id)[0].description_mon}"</span>
